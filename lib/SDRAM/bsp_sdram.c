...
uint8_t SDRAM_Test(void)
{
  ...
  for (counter = 0x00; counter < W9864G6KH_SIZE; counter++)
  {
    *(__IO uint8_t *)(SDRAM_BANK_ADDR + counter) = (uint8_t)0x0;
  }

  /* 向整个SDRAM写入数据  8位 */
  for (counter = 0; counter < W9864G6KH_SIZE; counter++)
  {
  ...
  /* 把SDRAM数据全部重置为0 */
  for (counter = 0x00; counter < W9864G6KH_SIZE / 2; counter++)
  {
    *(__IO uint16_t *)(SDRAM_BANK_ADDR + 2 * counter) = (uint16_t)0x00;
  }

  /* 向整个SDRAM写入数据  16位 */
  for (counter = 0; counter < W9864G6KH_SIZE / 2; counter++)
  {
